...
namespace ATI.Services.RabbitMQ
{
    ...
    public class EventbusManager : IDisposable, IInitializer
    {
        ...
        public async Task SubscribeAsync(
            ...
            [CanBeNull] Func<byte[], MessageProperties, MessageReceivedInfo, Task<Acknowledgements>> poisonHandler = null,
            ...
            string metricEntity)
        {
            ...
            Func<byte[], MessageProperties, MessageReceivedInfo, Task<AckStrategy>>
                ...
                return async (body, props, info) =>
                {
                    using (_metricsTracingFactory.CreateLoggingMetricsTimer(metricEntity ?? "Eventbus"))
                    {
                        ...
                        var handlerAcknowledgeResponse = await ExecuteWithPolicy(
                            async () => await handler.Invoke(body, props, info)
                        );
            ...
            Func<byte[], MessageProperties, MessageReceivedInfo, Task> HandlePoisonQueueMessages()
            {
                return async (body, props, info) =>
                {
                    using (_metricsTracingFactory.CreateLoggingMetricsTimer($"{metricEntity ?? "Eventbus"}-Poison"))
                    {
                        ...
                        await ExecuteWithPolicy(
                            async () => await poisonHandler.Invoke(body, props, info)
                        );
                    }
                };
            }
            ...
            private async Task PublishToPoisonQueueAsync(QueueExchangeBinding poisonQueueBinding, byte[] messageBody)
    {
        ...
        _busClient.Bind(delayExchange, poisonQueueBinding.Queue, poisonQueueBinding.RoutingKey);
        ...
        byte[] messageBody)
    {
        try
        {
            await _busClient.QueueDeclareAsync(
                ...
                c => c.WithArgument("x-dead-letter-exchange", mainQueue.Exchange.Name)
